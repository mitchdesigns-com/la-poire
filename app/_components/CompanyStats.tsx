...
export default function CompanyStats({
  ...
  return (
    <div className={`${fullWidth ? "bg-black py-[75px] mb-72 lg:mb-0" : ""}`}>
      <div
        className={`flex lg:flex-row flex-col-reverse	 flex-nowrap  gap-10 w-full items-center justify-evenly md:pr-[186px]  ${
          fullWidth ? "mx-auto container" : ""
        }`}>
        <div
          className={`${
            fullWidth ? "  pl-3 md:w-[461px] w-full" : "px-4  xl:w-460 w-full "
          }`}>
          <h3 className='mb-5  md:ml-0 text-m mb:text-xl text-white'>
            {numbersTitle}
          </h3>
          <p
            className={`${
        ...
        <div className='flex w-full items-stretch justify-center sm:gap-[17px] pt-20 md:justify-between md:gap-0 md:pt-0'>
          ...
          {sinceYear && (
            <StatItem value={sinceYear} label={t("since")} removeComma />
          )}
